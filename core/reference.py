...
def fileNameSWPC(*date, next_folder=False):
    ...
    return os.path.join(path_data, "reference", f"{date_.year + next_folder}_events",
...
def fileNameMonstein(*date):
    date_ = config.getDateFromArgs(*date)
    return os.path.join(path_data, f"reference/{date_.year}_monstein/{date_.year}{date_.month:02}events.txt")
