...
if __name__ == '__main__':
    ...
    args = [2022, 1, 1,  # date
            ...
            50000000     # step -> name of save file
            ]
    ...
    while date <= date_end:
        ...
        if os.path.isfile(analysis.filename(date, step=args[step])):
            ...
            print(" skipped ")
            continue
        data_ = analysis.loadData(date, step=2)
        data_step_4 = steps.thirdStep(data_, date, peak_limit=args[peak_limit], mask_frq=args[mask_frq], bin_time_w=args[bin_t_w])
