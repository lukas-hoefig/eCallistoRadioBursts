...
def getReference(*date, europeUT=True):
    ...
    try:
        ...
        print(f'no reference for {date_.strftime("%y-%m-%d")}')
...
def compareToRef(data: events.EventList, ref: events.EventList, *date):
    ...
    """
    ref_strong = events.EventList([i for i in ref if i.burst_type.endswith("II/2") or i.burst_type.endswith("II/3")], *date)
    ...
    for i in data:
        if i.inList(ref):
            ...
            false_positives += i
    return len(missed_swpc), len(event_match_all), len(false_positives) 
...
if __name__ == '__main__':
    ...
    file = config.path_script + f"/results/all_summed_results_{args[step]}.txt"
